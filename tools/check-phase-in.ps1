# Phase-in check: real Claude Code / Codex sessions on a fixture project,
# prompts that need a screen and prompts that don't, recording whether each agent used ARS.
# This makes real model calls on the owner's account. Run it only with their go-ahead.
...
param(
    ...
    [switch]$ScoreOnly,   # score the logs already in -Out again, without running any agent
    [string[]]$Only = @(),   # run just these cases, keeping the other logs
    [string]$Out = ''
)
...
if ($IncludeOwnerOpen) {
    ...
    # The app the agent builds is also the one the owner wants to use (a game they made): their words decide.
...
foreach ($case in $cases) {
    ...
    try {
        if ($Agent -eq 'claude') {
            ...
            @{ mcpServers = @{ ars = @{ type = 'stdio'; command = $bridge; args = @('--workspace', $ticket) } } } |
            ...
            $flags = @('-p', $case.prompt, '--mcp-config', $config, '--strict-mcp-config', '--output-format', 'stream-json',
                '--verbose', '--max-turns', '30', '--allowedTools', 'Bash,Read,Edit,Write,Glob,Grep,mcp__ars')
            ...
            $flags = @('exec', '--json', '--skip-git-repo-check', '--dangerously-bypass-approvals-and-sandbox',
                '-c', ('mcp_servers.ars.command="{0}"' -f $bridge.Replace('\', '\\')),
                '-c', ('mcp_servers.ars.args=["--workspace","{0}"]' -f $ticket.Replace('\', '\\')))
    ...
    # Which tools the agent reached for, from its own event stream: an ARS call counts only if
    ...
    foreach ($line in Get-Content -LiteralPath $log) {
        ...
        if ($Agent -eq 'claude') {
            foreach ($part in @($event.message.content)) {
                ...
                } elseif ($part.type -eq 'tool_result' -and -not $part.is_error -and "$($calls[$part.tool_use_id])" -like 'mcp__ars__*') {
    ...
    $used = @($tools | Where-Object { $_ -like 'mcp__ars__*' -and $_ -notmatch '__(status|release)$' }).Count -gt 0
