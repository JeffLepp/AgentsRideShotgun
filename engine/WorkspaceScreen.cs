...
/// (width,height). The owner's PC may have three monitors, and a workspace desktop shares their
/// monitor layout, so a program that remembers where it was last opens on whichever of them it
/// was on. Measured: a large WPF app opened at (-1684,119), on the monitor to the left
...
internal static class WorkspaceScreen
{
    ...
    /// the way ARS's own shell treats a position saved on a monitor that is no longer there;
