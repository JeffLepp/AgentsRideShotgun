...
This desktop executable exercises the copied engine and its packaged MCP bridge with two
...
Build from the repository root, after other engine builds have finished:

```powershell
dotnet build standalone/Deskweave/engine-probe/Deskweave.EngineProbe.csproj -c Release
```

Launch as a hidden desktop process, with a new absolute output directory each run:

```powershell
$deskweaveProbeExe = (Resolve-Path -LiteralPath 'standalone/Deskweave/engine-probe/bin/Release/net10.0-windows/Deskweave.Probe.exe').Path
$deskweaveProbeOutput = [IO.Path]::GetFullPath((Join-Path $PWD ('standalone/Deskweave/artifacts/engine-probe-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))))
